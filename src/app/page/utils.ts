...
async function addMosaicToImage(url, mosaicSize = 20) {
  const img = new Image();
...
/** UI to code post */
const figmaWebToPluginPost = (postInfo: {type: string, message: any}) => {
  parent.postMessage({ pluginMessage: postInfo }, '*')
}

export {
  addMosaicToImage,
  figmaWebToPluginPost
}